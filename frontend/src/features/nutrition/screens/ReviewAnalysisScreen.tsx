...
import { useAnalysis, useConfirmAnalysis, useDiscardAnalysis } from '../hooks';
...
export function ReviewAnalysisScreen({ navigation, route }: Props) {
  ...
  const discardMutation = useDiscardAnalysis();
  const analysis = analysisQuery.data;

  const discard = () => {
    if (discardMutation.isPending) return;
    // Server-side cleanup for a scan the user rejects — otherwise drafts leak.
    discardMutation.mutate(route.params.analysisId, {
        ...
        <View className="px-5 pb-12">
          ...
          {analysisQuery.isPending ? (
            ...
            <>
              ...
              <View className="mt-4">
                <Button
                  label={t('nutrition.discardAnalysis')}
                  variant="secondary"
